...
export const PayablesPage = () => {
  const { payables, setPayables, showForm, showMessage, setCurrentDetail, setActiveTab, addLog } = useContext(AppContext);
  ...
  const handlePayment = (item) => {
    showForm('登记付款', [{ name: 'amount', label: `本次付款 (最多: ¥${item.unpaidAmount})`, type: 'number' }, { name: 'voucherNo', label: '凭证号' }, { name: 'date', label: '付款日期', type: 'date' }], { date: getLocalDate() }, (form) => {
      const pAmt = parseFloat(form.amount);
      if (isNaN(pAmt) || pAmt <= 0 || pAmt > parseFloat(item.unpaidAmount)) return showMessage('错误', '金额不合法或超出应付总额！');
      const unpd = (parseFloat(item.totalAmount) - (parseFloat(item.paidAmount) + pAmt)).toFixed(2);
      setPayables(payables.map(r => r.id === item.id ? { ...r, paidAmount: (parseFloat(item.paidAmount) + pAmt).toFixed(2), unpaidAmount: unpd, status: Number(unpd) <= 0 ? '已结清' : '部分结清', payments: [...(r.payments||[]), { date: form.date, amount: pAmt.toFixed(2), voucherNo: form.voucherNo }] } : r));
      addLog('应付管理', '登记付款', `单号: ${item.orderNo}, 金额: ¥${pAmt.toFixed(2)}`);
    });
  };

  const handleEdit = (item) => {
    showForm('编辑应付记录', [{ name: 'date', label: '发生日期', type: 'date' }, { name: 'supplier', label: '供应商名称' }], item, (formData) => {
      if (!formData.date || !formData.supplier) return showMessage('警告', '必填项不能为空');
      setPayables(payables.map(r => r.id === item.id ? { ...r, ...formData } : r));
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm p-4 sm:p-6 min-h-full flex flex-col border border-slate-100">
      ...
      <div className="flex mb-6"><div className="relative flex-1 sm:max-w-md"><input type="text" value={search} onChange={e=>{setSearch(e.target.value);setPage(1);}} placeholder="单号/供应商" className="w-full border rounded-xl px-4 py-2.5 pl-10 text-sm bg-slate-50 focus:bg-white"/><Search className="absolute left-3.5 top-3 text-slate-400" size={18}/></div></div>
